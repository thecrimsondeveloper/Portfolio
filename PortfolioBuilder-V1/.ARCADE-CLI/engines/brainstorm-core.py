...
DEFAULT_INPUT = SESSIONS_DIR / "summary_brainstorming.json"
DEFAULT_OUTPUT = SESSIONS_DIR / "final_brainstorming.json"
...
def build_objective_text(player_actions: list[str], enemy_types: list[str], progression_goal: list[str]) -> str:
    ...
    return (
        f"Use {primary_action} and {secondary_action} to resolve {primary_enemy}-driven pressure, "
        f"complete the active objective chain, and turn each run into {progression_phrase} progress."
    )
...
def choose_runtime_template(summary_data: dict, final_spec: dict) -> tuple[str, str, str]:
    ...
    category_text = " ".join(
        " ".join(str(word) for word in category_words(summary_data, category))
        for category in (
            "Genre Seeds",
            "Camera Mode Seeds",
            "Arena Shape Seeds",
    ...
    explicit_text = f"{category_text} {text}"
    if any(word in explicit_text for word in ("route runner", "runner", "delivery", "checkpoint", "finish line", "lane", "sprint", "momentum", "parkour", "chase", "scrolling")):
        return "route_runner", "route runner", "lane route"
    if any(word in explicit_text for word in ("relay", "signal", "reconnect", "circuit", "network", "node", "link", "sequence", "chain reaction")):
        return "relay_chain", "relay chain", "node network"
    if any(word in explicit_text for word in ("maze", "map", "library", "cavern", "ruin", "search", "artifact", "vault", "extraction", "key", "escape route")):
        return "extraction_maze", "extraction maze", "maze"
    if any(word in explicit_text for word in ("stealth", "infiltration", "crouch", "avoid", "sneak", "caught", "alarm", "visibility", "shadow")):
...
def build_final_spec(summary_data: dict, source_summary_path: Path, step_answers: list[dict[str, object]], random_injections: list[dict[str, object]], exploration_loops: list[dict[str, object]], model: str, endpoint: str, temperature: float) -> dict[str, object]:
    ...
    canonical_description = one_line_pitch or (
        ...
        f"solve {primary_enemy}-driven pressure, and turn each run into {progression_anchor} progress."
    )
    ...
    return {
        ...
        "buildPacket": {
            ...
            "overlay": {
                ...
                "winBody": "The completion condition is live and the objective chain is stable. Press R to run it again.",
