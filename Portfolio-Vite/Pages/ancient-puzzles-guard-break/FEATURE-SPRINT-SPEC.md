...
- Active packet reason: Enemy threat is necessary to make movement and pulses meaningful.
- Active sprint objective: Implement the one enemy archetype slice so the player can reliably feel ember under sentry.
- Active player promise: The player immediately feels how one enemy archetype improves the route and unlocks unify.
- Current pass goal: Implement a patrol sentry enemy that reacts to pulses and can be stunned.
