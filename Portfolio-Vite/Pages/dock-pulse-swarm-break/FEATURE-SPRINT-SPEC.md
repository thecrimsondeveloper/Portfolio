...
- Active packet reason: A single commander-type enemy directly reinforces route ownership, bridge control, and repair timing without requiring a full enemy-system rewrite.
- Active sprint objective: Implement the one enemy archetype slice so the player can reliably feel spike under commander.
- Active player promise: The player immediately feels how one enemy archetype improves the route and unlocks upgrade.
- Current pass goal: Add one readable commander archetype that locks bridge lanes and forces rerouting during the relay-repair run.
