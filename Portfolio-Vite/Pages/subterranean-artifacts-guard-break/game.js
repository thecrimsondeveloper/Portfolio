...
class ArenaGame {
  ...
  updateRelics(delta) {
	this.relics.forEach((relic, index) => {
	  ...
	  if (distance < 1.65) {
		...
		this.relicCount += 1;
		this.setStatus(`${relic.label} secured. Exit charge increased.`, 1.0);
  ...
  updateGoal() {
	...
	if (distance < this.goal.radius) {
	  this.finishRun('Route Cleared', 'The exit gate is live and the relic chain is stable. Press R to run it again.');
	}
  }
  ...
  updateHud(delta) {
	...
	this.metrics.textContent = `Health ${Math.ceil(this.player.health)} | Relics ${this.relicCount}/${this.goal.unlockRelics} | Time ${Math.ceil(this.timeRemaining)}s | Dash ${this.player.dashCooldown > 0 ? this.player.dashCooldown.toFixed(1) + 's' : 'ready'} | Pulse ${this.player.pulseCooldown > 0 ? this.player.pulseCooldown.toFixed(1) + 's' : 'ready'}`;
  }
