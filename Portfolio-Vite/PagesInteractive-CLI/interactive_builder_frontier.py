...
PROMPT_CONFIG_FILE = ROOT / "PagesInteractive-CLI" / "interactive_builder_frontier.json"
BRAINSTORM_CLI_FILE = ROOT / "PagesBrainstorm-CLI" / "brainstorm_cli.py"
CHAINSUMMARY_CLI_FILE = ROOT / "PagesBrainstorm-CLI" / "chainsummary_cli.py"
SEED_FETCHER_CLI_FILE = ROOT / "PagesBrainstorm-CLI" / "seed_fetcher_cli.py"
...
def render_stage_prompt(stage_id: str, variables: dict) -> str:
	...
	for pack_name in stage_prompt.get("includeContextPacks", []):
		pack = context_packs.get(pack_name, [])
		if pack:
...
def confirm_unique_idea(title: str, description: str, pages: list[dict]) -> bool:
...
def review_html_loop(target_dir: Path, title: str, description: str, concept: dict) -> None:
...
def convert_final_brainstorm_to_concept(title: str, description: str, objective: str, combination: dict, brainstorm_artifacts: dict) -> dict:
	...
	if len(loop_beats) < 4:
		loop_beats = merge_unique_strings(
			...
			[
				...
				"Use the core verb set to survive the active wave.",
				"Convert the current room into progression gain.",
				"Unlock the exit and escape the arena.",
			],
			limit=6,
		)
	...
	status_message = coerce_text(
		...
		coerce_text(hud.get("objective"), f"Collect relics, survive {join_inline(pressure[:2], 'enemy pressure')}, and unlock the exit gate."),
	)
...
def main() -> None:
	...
	while True:
		...
		if confirm_unique_idea(game_title, game_description, existing_pages):
			break
	...
	create_story_json(target_dir, folder_name, game_title, game_description, seed_payload, concept, design_text, chapters, fun_assessment, fun_needs, feature_packets)
	record_frontier_stage("stage-6-story-and-scaffold-materialization", f"Created story payload and runtime scaffold for {runtime_template(concept.get('game_design_profile', {}))}.")

	print("\nSTAGE 7: Run one bundled HTML shell review pass.")
	create_game_script(target_dir)
	review_html_loop(target_dir, game_title, game_description, concept)
